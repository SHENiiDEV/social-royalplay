...
export default function AdminDashboard({
    ...
    const [selectedUserRtp, setSelectedUserRtp] = useState({});
    const [loadingRtpUserId, setLoadingRtpUserId] = useState(null);
    const [rtpFeedback, setRtpFeedback] = useState(null);
    ...
    const handleRtpChange = async (userId, rtpVal) => {
        ...
        try {
            ...
            setRtpFeedback({ type: 'error', message: 'Network connection error.' });
        } finally {
    ...
    const handleAdjustBalance = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok && data.success) {
                setRtpFeedback({ type: 'success', message: data.message });
                setAdjustUser(null);
                setAdjustAmount('');
    ...
    return (
        <div className="min-h-screen bg-[#07090e] text-slate-100 flex flex-col font-sans">
            <Head title="Casino Admin & RTP Control — RoyalPlay" />
            ...
            <header className="bg-[#0c1018] border-b border-slate-800 px-4 sm:px-8 py-4 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-40 shadow-2xl">
                    ...
                    <div className="flex items-center gap-2 pl-2 border-l border-slate-800">
                        ...
                        <div>
                            <h1 className="text-base font-black text-white flex items-center gap-2 leading-none">
                                <span>CROWDPLAY Management</span>
                            ...
                            <span className="text-[11px] text-slate-400 font-mono">
                                Nexus GGR Gold API • Seamless Wallet
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <Link
                        href="/admin/users"
                        className="px-4 py-2 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-black text-xs flex items-center gap-2 shadow-lg shadow-red-600/30 transition"
                    >
                        <ShieldAlert className="w-3.5 h-3.5" />
                        <span>Manage Users & Bans (/admin/users)</span>
                    </Link>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 w-full space-y-8 flex-1">
                ...
                {rtpFeedback && (
                    <div className={`p-4 rounded-2xl border text-xs font-bold flex items-center justify-between ${
                        rtpFeedback.type === 'success'
                            ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300'
                            : 'bg-rose-500/20 border-rose-500/40 text-rose-300'
                    }`}>
                        <div className="flex items-center gap-2">
                            {rtpFeedback.type === 'success' ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                            <span>{rtpFeedback.message}</span>
                        </div>
                        <button onClick={() => setRtpFeedback(null)} className="text-slate-400 hover:text-white">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {/* Total Turnover / Bets */}
                    <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 shadow-lg">
                        <div className="flex items-center justify-between text-slate-400 text-xs font-bold uppercase tracking-wider">
                            <span>Total Turnover (Bets)</span>
                            <DollarSign className="w-4 h-4 text-amber-400" />
                        </div>
                        <div className="text-2xl sm:text-3xl font-black font-mono text-white mt-2">
                            {formatEuro(metrics.totalBets)}
                        </div>
                        <div className="text-[11px] text-slate-400 mt-1 font-mono">
                            Total Spins: <strong className="text-slate-200">{metrics.totalSpins}</strong>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 shadow-lg">
                        <div className="flex items-center justify-between text-slate-400 text-xs font-bold uppercase tracking-wider">
                            <span>Total Wins Paid</span>
                            <TrendingUp className="w-4 h-4 text-emerald-400" />
                        </div>
                        <div className="text-2xl sm:text-3xl font-black font-mono text-emerald-400 mt-2">
                            {formatEuro(metrics.totalWins)}
                        </div>
                        <div className="text-[11px] text-slate-400 mt-1 font-mono">
                            Realized RTP: <strong className="text-emerald-300">{metrics.realizedRtp}%</strong>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 shadow-lg">
                        <div className="flex items-center justify-between text-slate-400 text-xs font-bold uppercase tracking-wider">
                            <span>Net GGR (Profit)</span>
                            <Zap className="w-4 h-4 text-amber-400" />
                        </div>
                        <div className={`text-2xl sm:text-3xl font-black font-mono mt-2 ${
                            metrics.netGgr >= 0 ? 'text-amber-400' : 'text-rose-400'
                        }`}>
                            {formatEuro(metrics.netGgr)}
                        </div>
                        <div className="text-[11px] text-slate-400 mt-1 font-mono">
                            Casino Hold: <strong className="text-amber-300">{metrics.holdPercent}%</strong>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 shadow-lg">
                        <div className="flex items-center justify-between text-slate-400 text-xs font-bold uppercase tracking-wider">
                            <span>Jackpot & Community</span>
                            <Users className="w-4 h-4 text-cyan-400" />
                        </div>
                        <div className="text-2xl sm:text-3xl font-black font-mono text-cyan-300 mt-2">
                            {formatEuro(metrics.jackpotPool)}
                        </div>
                        <div className="text-[11px] text-slate-400 mt-1 font-mono">
                            Total Players: <strong className="text-slate-200">{metrics.totalUsers}</strong>
                        </div>
                    </div>
                ...
                <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 md:p-6 shadow-xl space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-base font-extrabold text-white">Daily Turnover & GGR Trends</h2>
                        </div>
                    ...
                    <div className="h-64 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={dailyTrends}>
                                ...
                                />
                                <Area type="monotone" dataKey="bets" name="Bets (SC)" stroke="#f59e0b" fillOpacity={1} fill="url(#betsGrad)" strokeWidth={2} />
                                <Area type="monotone" dataKey="wins" name="Wins (SC)" stroke="#10b981" fillOpacity={1} fill="url(#winsGrad)" strokeWidth={2} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>

                {/* RTP Control Center & Players Table */}
                <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 md:p-6 shadow-xl space-y-4">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h2 className="text-base font-extrabold text-white">
                                    Slot RTP & Player Enforcement (Block & Police Notice)
                                </h2>
                                <p className="text-xs text-slate-400">
                                    Control individual player payout RTP (95% - 999%) or immediately trigger fraud block and law enforcement dispatch.
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left text-xs">
                            <thead className="bg-slate-900/80 text-slate-400 uppercase font-mono text-[10px] border-b border-slate-800">
                                <tr>
                                    ...
                                    <th className="py-3 px-4">Change RTP</th>
                                    <th className="py-3 px-4 text-right">Enforcement Actions</th>
                                </tr>
                            </thead>
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    {u.is_banned && (
                                                        <span className="text-[9px] font-mono text-rose-400 flex items-center gap-1">
                                                            ...
                                                            ⛔ BLOCKED (POLICE DISPATCH)
                                                        </span>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="py-3 px-4 text-slate-400">{u.user_code}</td>
                ...
                {/* Live Webhook Transactions Audit */}
                <div className="bg-[#0f1420] border border-slate-800 rounded-3xl p-5 md:p-6 shadow-xl space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Activity className="w-5 h-5 text-emerald-400" />
                            <h2 className="text-base font-extrabold text-white">
                                Live Nexus GGR Webhook Transactions Audit
                            </h2>
                        </div>
                        <span className="text-xs font-mono text-slate-400">Latest 25 Spins</span>
                    </div>
                        ...
                        <table className="w-full text-left text-xs">
                            <thead className="bg-slate-900/80 text-slate-400 uppercase font-mono text-[10px] border-b border-slate-800">
                                <tr>
                                    ...
                                    <th className="py-3 px-4">Net GGR</th>
                                ...
                                {recentTransactions.length === 0 ? (
                                    <tr>
                                        <td colSpan="9" className="text-center py-6 text-slate-500">
                                            No transactions recorded yet. Launch a slot game to test spins!
                                        </td>
                                    </tr>
                                ) : (
                                    recentTransactions.map((tx) => (
                                        <tr key={tx.id} className="hover:bg-slate-900/40 transition">
                                            ...
                                            <td className="py-2.5 px-4 font-mono text-slate-300 text-[11px] max-w-[120px] truncate">
                                                {tx.txn_id}
                                            </td>
                                            <td className="py-2.5 px-4 font-sans font-bold text-white">
                                                {tx.user?.name || tx.user_code}
                                            </td>
                                            <td className="py-2.5 px-4 text-slate-400 text-[11px]">
                                                <span className="bg-slate-800 px-1.5 py-0.5 rounded text-[10px] text-amber-400 mr-1">
                                                    {tx.provider_code}
                                                </span>
                                                {tx.game_code}
                                            </td>
                                            <td className="py-2.5 px-4 text-slate-300 font-bold">
                                                SC {Number(tx.bet_money).toFixed(2)}
                                            </td>
                                            <td className="py-2.5 px-4 font-bold text-emerald-400">
                                                +SC {Number(tx.win_money).toFixed(2)}
                                            </td>
                                            <td className={`py-2.5 px-4 font-bold ${
                                                tx.net_money <= 0 ? 'text-amber-400' : 'text-rose-400'
                                            }`}>
                                                SC {(-1 * Number(tx.net_money)).toFixed(2)}
                                            </td>
                                            <td className="py-2.5 px-4 text-slate-300">
                                                SC {Number(tx.user_balance_after).toFixed(2)}
                                            </td>
                                            <td className="py-2.5 px-4 text-right">
                                                <button
                                                    onClick={() => setInspectedPayload(tx.raw_payload || JSON.stringify(tx, null, 2))}
                                                    className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white"
                                                    title="View Raw JSON Webhook Payload"
                                                >
            ...
            {inspectedPayload && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="w-full max-w-2xl bg-[#0f141f] border border-slate-800 rounded-3xl p-6 shadow-2xl flex flex-col max-h-[85vh]">
                        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                            <h3 className="font-bold text-base text-white font-mono">Raw Webhook JSON Payload</h3>
                            <button onClick={() => setInspectedPayload(null)} className="text-slate-400 hover:text-white">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto my-4 p-4 rounded-2xl bg-black/80 border border-slate-800">
                            <pre className="font-mono text-xs text-amber-300 whitespace-pre-wrap">
                                {typeof inspectedPayload === 'string'
                                    ? JSON.stringify(JSON.parse(inspectedPayload || '{}'), null, 2)
